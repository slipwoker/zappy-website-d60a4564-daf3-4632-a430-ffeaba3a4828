document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    function getFieldLabel(field) {
      ...
      if (!label) {
        label = field.closest('.form-group, .field-wrapper, div')
          ? field.closest('.form-group, .field-wrapper, div').querySelector('label')
          : null;
      }
      return label ? label.textContent.trim() : (field.name || field.id || 'This field');
    }

    function showError(field, message) {
      field.classList.add('is-invalid', 'field-error');
      field.classList.remove('is-valid', 'field-success');
      field.setAttribute('aria-invalid', 'true');

      let errorEl = document.getElementById('error-' + field.name) ||
        field.parentElement.querySelector('.form-error, .error-message');
    ...
    function clearError(field) {
      field.classList.remove('is-invalid', 'field-error');
      field.classList.add('is-valid', 'field-success');
    ...
    validatableFields.forEach(function (field) {
      ...
      field.addEventListener('input', function () {
        if (field.classList.contains('is-invalid') || field.classList.contains('field-error')) {
          validateField(field);
        }
      });
    });
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .then(function () {
          contactForm.reset();
          validatableFields.forEach(function (field) {
            field.classList.remove('is-valid', 'is-invalid', 'field-success', 'field-error');
          });

          let successMsg = contactForm.querySelector('.form-success, .success-message');
          if (!successMsg) {
            successMsg = document.createElement('div');
            successMsg.className = 'form-success success-message';
  ...
  if (animatedElements.length > 0) {
    ...
    defaultStyles.textContent = [
      '.animate, .fade-in, .fade-in-up, .scroll-animate, .reveal, [data-animate] {',
      '  opacity: 0;',
      '  transform: translateY(24px);',
      '  transition: opacity 0.6s ease, transform 0.6s ease;',
      '}',
      '.animate.is-visible, .fade-in.is-visible, .fade-in-up.is-visible,',
      '.scroll-animate.is-visible, .reveal.is-visible, [data-animate].is-visible {',
      '  opacity: 1;',
      ...
      animatedElements.forEach(function (el, index) {
        if (!el.dataset.delay && !el.dataset.animateDelay) {
          const siblings = el.parentElement
            ? Array.from(el.parentElement.children).filter(function (child) {
              return child.classList.contains('animate') ||
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "sky2fly משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd60a4564-daf3-4632-a430-ffeaba3a4828',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd60a4564-daf3-4632-a430-ffeaba3a4828';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd60a4564-daf3-4632-a430-ffeaba3a4828';
